...
ENV MYSQL_PASSWORD  Hemiao769
ENV DATABASE_NAME ya-music
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/ya-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/ya-music-0.0.1.jar", "--spring.profiles.active=test"]